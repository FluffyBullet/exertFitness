...
<table>
<td width="150px" height="150px"><img src="assets/images/exert_fit_logo.png" alt="Exert Fit logo" height="150px"><td>
<td>Live website link: <a href="https://fluffybullet.github.io/exertFitness">Please click here.</td>
</table>
...
This amendment was then reflected to the view modes.<br>
...
| Button     | Before       | After      |
|-------|-------|-------|
|Entry chart | <img src="assets\images\readme_files\step_one_button_before.png" alt="button one display before being pressed"> | <img src="assets\images\readme_files\step_one_button_after.png" alt="button one display after being pressed"> |
...
Learning with JavaScript has been tough from the beginning, but finding more use of dev tools and console log there was an gradual level of understanding and it's application. The website features three main sections coded or reliant on JavaScript to function as intended.

1. Nav Bar
When a specific max-width is trigged for display of the website, JavaScript reduce the navigation bar from three hyper links (separate pages) to the current page and hamburger menu. Clicking on the hamburger menu acts as a dropdown, which uses JavaScripts onclick function.<br>
...
At the bottom of the tile, two buttons adjust the numeric value inserted to the variable for tile selection. For this, each offered a function of on click then add or subtract 1, updating the numeric value outside the function. 
This variable is then called with .style function of JavaScript to edit it's display function to Block, navigating through the Parent divisions children.

3. Calculator entry and analytics  
...
This function retrieves the mentioned values storing variables, then performs an if statement dependent on the users entry from the dropdown input.<br>
...
Pages included a duplication of Author code in the head element - as this was copied and pasted from other pages.
...
2. Testing of written code/language
    * This project includes a total of 4 languages used for display and instructions to the computer:
    ...
    <br>
    2. CSS, Tested by W3 Validator - Jigsaw<br>
    Results, No errors found (8/8/22):<br>
    <img src="assets/images/readme_files/exert_fit_jigsaw_test.png" alt="Jigsaw validator displaying no errors result" height="150px"><br>
    3. Javascript, Tested by JSHint
        * Previously listed as one document, this has since been divided into two separate files. Function of navigating the testimonies was attempted to be ran on the calculator page, finding an error was displayed on load. To simplify this, the two button functions to cycle through has been placed on it's own dedicated .js file.<br>

    | Page | JSHint display | Comments |
    |----|-----|-----|
    |home.js| <img src="assets/images/readme_files/jshint_home_test.png" alt="JSHint's report on script for index page" height="150px">| N/A |
    |data2.js| <img src="assets/images/readme_files/jshint_data_test.png" alt="JSHint's report on script for calculator page"> | 
    Warning issued - "Do not use 'new' for side effects". This relates to code copied from ploty pages for creation of the graph. To avoid corropting the graph data this has remained.<br>
    undefined variables - Chart, xValues, yValues and plotly  are also imported code from plotly website for the graph creation. Similar to the above, the code has remained untouched to avoid errors.<br>
    morningSnack and afternoonSnack - Values to be updated on the graph at a later stage, these variables extract and log the correct values but not displayed correctly so retracted from the working model but still logged.<br>
    Unused variables relate to the functions called from buttons on the original calculator page. This is used to display use of different types of function calls, in this instance "onclick", whilst the home page uses addEventListener"|
    4. markdown langage - README, Tested by Microsoft Word<br>
    All entries added into this README file was extracted to a Microsoft Word document [as of 8/8/22, total wordcount - 3,913 words] to filter incorrect word spellings. Edits were made during the logging of this chapter, but found all other incorrect words were either tags or name references. 
...
### Current Bugs
 ...
 10. If entered target field with small or undividable values, the system calculates to decimal placements:<br>
...
- Testing validators
    * W3 Validator for HTML - https://validator.w3.org/#validate_by_input
    * W3 Jigsaw Validator for CSS - https://jigsaw.w3.org/css-validator/validator
    * JShint Validator for JavaScript - https://jshint.com
    * Microsoft Word for .md document
## <u>References</u>
